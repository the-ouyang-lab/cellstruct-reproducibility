...
Permuted = function (pbmc)
{
  ...
  if(Seurat::DefaultAssay(pbmc) == 'RNA'){
    ...
    X=pbmc@assays$SCT@scale.data
    X_permuted = pbmc.permuted@assays$SCT@scale.data
  }
  ...
  if(Seurat::DefaultAssay(pbmc) == 'RNA'){
    ...
    pbmc.permuted@assays$SCT@scale.data = X_permuted
  }
...
Cell.Similarity.UMAP = function(Euc_distances,Euc_distances_permuted,UMAP_distances,UMAP_distances_permuted,percent)
{
  ...
  ## (added by me) for each cell, correlating the distances between UMAP (ordered by PCA closest 50%) with the closest 50% of UMAP
...
scDEED = function(input_data, num_pc, n_neighbors = c(seq(from=5,to=30,by=1),35,40,45,50), min.dist = seq(0.1,0.9, by = 0.2), similarity_percent = 0.5, visualization = FALSE, use_method = "umap",
  ...
  if(use_method == "umap"){
    ...
    #added by me
    input_data$rho_UMAP <- similarity_score_UMAP$rho_UMAP
    input_data$scDEED.classify.UMAP <- rep("Neither cells", ncol(input_data))
    input_data$scDEED.classify.UMAP[ClassifiedCells_UMAP$UMAP_badindex] <- "Dubious cells"
    input_data$scDEED.classify.UMAP[ClassifiedCells_UMAP$UMAP_goodindex] <- "Trustworthy cells"
    print(paste0("cellstruct:",date()))
    input_data$UMAP_gs <- calcGS(input_data@reductions[["pca"]]@cell.embeddings, input_data@reductions[["umap"]]@cell.embeddings)
    input_data.permuted$UMAP_gs <- calcGS(input_data.permuted@reductions[["pca"]]@cell.embeddings, input_data.permuted@reductions[["umap"]]@cell.embeddings)
    print(date())
    cellstruct.ClassifiedCells_UMAP<-Cell.Classify.UMAP(input_data$UMAP_gs,input_data.permuted$UMAP_gs)
    input_data$cellstruct.classify.UMAP <- rep("Neither cells", ncol(input_data))
    input_data$cellstruct.classify.UMAP[cellstruct.ClassifiedCells_UMAP$UMAP_badindex] <- "Dubious cells"
    input_data$cellstruct.classify.UMAP[cellstruct.ClassifiedCells_UMAP$UMAP_goodindex] <- "Trustworthy cells"
    ...
    if(visualization == TRUE){
      ...
      if(optimize_neib == TRUE && optimize_min == FALSE){
        ...
        output <-list(input_data,cell_list[ClassifiedCells_UMAP$UMAP_badindex], cell_list[ClassifiedCells_UMAP$UMAP_goodindex], 
                      bad_graph, trust_graph)
        names(output) <- c("object","list of dubious cells",
  ...
  if(use_method == "tsne"){
    ...
    best_para <- perplexity_score # added by me
    print(paste0("noOPTIMIZE:",best_para)) # added by me
    ...
    #added by me
    input_data$rho_tSNE <- similarity_score_tSNE$rho_tSNE
    input_data$scDEED.classify.tSNE <- rep("Neither cells", ncol(input_data))
    input_data$scDEED.classify.tSNE[ClassifiedCells_tSNE$tSNE_badindex] <- "Dubious cells"
    input_data$scDEED.classify.tSNE[ClassifiedCells_tSNE$tSNE_goodindex] <- "Trustworthy cells"
    print(paste0("cellstruct:",date()))
    input_data$TSNE_gs <- calcGS(input_data@reductions[["pca"]]@cell.embeddings, input_data@reductions[["tsne"]]@cell.embeddings)
    input_data.permuted$TSNE_gs <- calcGS(input_data.permuted@reductions[["pca"]]@cell.embeddings, input_data.permuted@reductions[["tsne"]]@cell.embeddings)
    print(date())
    cellstruct.ClassifiedCells_tSNE<-Cell.Classify.tSNE(input_data$TSNE_gs,input_data.permuted$TSNE_gs)
    input_data$cellstruct.classify.tSNE <- rep("Neither cells", ncol(input_data))
    input_data$cellstruct.classify.tSNE[cellstruct.ClassifiedCells_tSNE$tSNE_badindex] <- "Dubious cells"
    input_data$cellstruct.classify.tSNE[cellstruct.ClassifiedCells_tSNE$tSNE_goodindex] <- "Trustworthy cells"
    return(input_data)
